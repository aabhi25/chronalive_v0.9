...
export default function TimetableGridSimple() {
  ...
  useEffect(() => {
    ...
    if (classParam && classes.length > 0) {
      // Find class by grade number (e.g., URL param "1" matches "Class 1-A")
      const targetClass = classes.find(c => c.grade === classParam);
      ...
      <div className="p-6">
        {!selectedClass ? (
          <div className="text-center py-8 text-muted-foreground border rounded-lg bg-muted/20">
            ...
            <p className="text-sm">
              {!isWorkingDay() && `${selectedDate.toLocaleDateString('en-US', { weekday: 'long' })} is not a working day`}
            </p>
          </div>
