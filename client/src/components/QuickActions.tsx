...
export default function QuickActions() {
  ...
  const generateTimetableMutation = useMutation({
    mutationFn: async () => {
      console.log("[TIMETABLE GENERATION] Starting mutation...");
      ...
      console.log("[TIMETABLE GENERATION] Response status:", response.status);
      const data = await response.json();
      console.log("[TIMETABLE GENERATION] Response data:", data);
      return data;
    },
    onSuccess: (data) => {
      console.log("[TIMETABLE GENERATION] onSuccess called with data:", data);
      if (data.success) {
        console.log("[TIMETABLE GENERATION] Showing success toast with message:", data.message);
        toast({
          title: "Success",
        ...
        queryClient.invalidateQueries({ 
          predicate: (query) => {
            ...
            return queryKey[0] === "/api/timetable/detailed" || 
                   queryKey[0] === "/api/timetable/global" ||
        ...
        console.log("[TIMETABLE GENERATION] Showing failure toast with message:", data.message);
        toast({
    ...
    onError: (error) => {
      console.log("[TIMETABLE GENERATION] onError called with error:", error);
      toast({
        title: "Error",
